...
namespace Spindles {
    ...
    void Spindle::setupSpeeds(uint32_t max_dev_speed) {
        ...
        _up_max_delta_dev_speed   = max_dev_speed * _spinup_max_percent / 100;
        _down_max_delta_dev_speed = max_dev_speed * _spindown_max_percent / 100;
    }
    ...
    uint32_t Spindle::spindleDelayMsecs(SpindleState state, SpindleSpeed speed) {
        ...
        if (down) {
            return down < maxSpeed() ? _spindown_ms * down / maxSpeed() : _spindown_ms;
        }
        if (up) {
            return up < maxSpeed() ? _spinup_ms * up / maxSpeed() : _spinup_ms;
        }
        return 0;
    }

    void Spindle::spindleDelay(SpindleState state, SpindleSpeed speed) {
        delay(spindleDelayMsecs(state, speed));
